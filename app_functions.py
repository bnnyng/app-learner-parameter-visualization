...
def learner_optim(function_i, true_x, train_idxs, params, n_iter=600):
    # Get closed form of function
    A, B, C, D = function_i['params']
    target_fn = lambda x: A * torch.sin(B * x - C) + D

    train_xs = torch.tensor([true_x[i].item() for i in train_idxs])
...
def evaluate_params(function_i, true_x, train_idxs, params, plot=True, show_known_points=True, plot_title=''):
    A, B, C, D = function_i['params']
    target_fn = lambda x: A * torch.sin(B * x - C) + D
